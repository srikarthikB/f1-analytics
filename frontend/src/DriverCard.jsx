...
function DriverCard({ driver }) {
  ...
  return (
    <div
      ...
      className={`
        group relative w-56 cursor-pointer overflow-hidden rounded-2xl
        ...
        ${dark
          ...
          : `bg-white
             ...
             hover:shadow-[0_0_28px_rgba(239,68,68,0.12),0_8px_24px_rgba(0,0,0,0.1)]`
        }
      `}
    >
      {/* Top accent bar */}
      ...
      <div className="absolute top-0 right-0 w-12 h-12 overflow-hidden pointer-events-none">
        <div className={`absolute -top-1 -right-1 w-10 h-10 rotate-45 border
          ${dark ? "bg-red-600/10 border-red-500/20" : "bg-red-50 border-red-200/60"}`} />
      </div>
      ...
      <div className="px-5 pt-5 pb-6 flex flex-col gap-3">
        ...
        {driver.number && (
          <div className={`text-[3.5rem] font-black leading-none tracking-tighter select-none
            ...
            ${dark
              ...
              : "text-zinc-900/[0.04] group-hover:text-zinc-900/[0.08]"
            }`}>
            {driver.number}
          </div>
        )}
        ...
        <div className="flex flex-col gap-0.5">
          {driver.firstName && (
            <span className={`text-[10px] font-semibold tracking-[0.18em] uppercase font-mono
        ...
        <div className="flex items-center justify-between">
          {driver.team && (
            <span className={`text-[10px] uppercase tracking-[0.15em] font-semibold
              truncate max-w-[110px] font-mono
              ${dark ? "text-zinc-500" : "text-zinc-400"}`}>
              {driver.team}
            </span>
          )}
          {driver.points !== undefined && (
            <div className="flex items-baseline gap-1">
              ...
              <span className={`text-[9px] uppercase tracking-widest font-mono
                ${dark ? "text-zinc-600" : "text-zinc-400"}`}>pts</span>
            </div>
          )}
        </div>
      </div>
